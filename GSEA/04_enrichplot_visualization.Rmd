---
title: "04_enrichplot_visualization"
...
Note: Several visualization methods were first implemented in `DOSE` and rewrote from scratch using `ggplot2`. If you want to use the [old methods](https://www.biostars.org/p/375555), you can use the [doseplot](https://github.com/GuangchuangYu/doseplot) package.

## Bar Plot

Bar plot is the most widely used method to visualize enriched terms. It depicts the enrichment scores (*e.g.* p values) and gene count or ratio as bar height and color (Figure \@ref(fig:Barplot)A). Users can specify the number of terms (most significant) or selected terms (see also the [FAQ](#showing-specific-pathways)) to display via the `showCategory` parameter.
...
<!-- ```{r Barplot1, fig.cap="(ref:Barplotcap)", fig.height=12, fig.scap="(ref:Barplotscap)", fig.width=10, eval=F} -->
...
<!-- ```{r Barplot2, fig.cap="(ref:Barplotcap)", fig.height=6, fig.scap="(ref:Barplotscap)", fig.width=8} -->
...
```{r Barplot, fig.height=9, fig.width=12, fig.cap="(ref:Barplotcap)", fig.scap="(ref:Barplotscap)", echo=FALSE}
...
```{r Dotplotcap, fig.width=12, fig.height=16, fig.cap="(ref:Dotplotcap)", fig.scap="(ref:Dotplotscap)", echo=FALSE}
...
```{r Networkplot, fig.width=24, fig.height=12, fig.cap="(ref:Networkplotcap)", fig.scap="(ref:Networkplotscap)"}
...
```{r Heatplot, fig.width=14, fig.height=6, fig.cap="(ref:Heatplotcap)", fig.scap="(ref:Heatplotscap)"}
...
```{r treeplot, fig.width=15, fig.height=11, fig.cap="(ref:treeplotcap)", fig.scap="(ref:treeplotscap)"}
...
p4 <- emapplot(ego, cex.params = list(category_node = 1.5), layout.params = list(layout = "kk"), showCategory = 10) 
cowplot::plot_grid(p1, p2, p3, p4, ncol=2, labels=LETTERS[1:4])
```
...
# upsetplot(kk2) ## 由于kk2中并无结果导致报错，故注释
```
...
```{r ridgeplot, fig.width=12, fig.height=18, message=FALSE, fig.cap="(ref:ridgeplotcap)", fig.scap="(ref:ridgeplotscap)"}
ridgeplot(ego2)
```
...
```{r gsearank2, fig.width=20, fig.height=12, fig.cap="(ref:gsearank2cap)", fig.scap="(ref:gsearank2scap)"}
library(ggplot2)
library(cowplot)
...
pp <- lapply(1:3, function(i) {
    ...
    gsearank(ego2, i, title) + xlab(NULL) +ylab(NULL) +
        annotate("text", 150, ego2[i, "enrichmentScore"] * .75, label = lab, hjust=0, vjust=0)
})
...
```{r pmcplot, fig.width=10, fig.height=10, fig.cap="(ref:pmcplotcap)", fig.scap="(ref:pmcplotscap)"}
