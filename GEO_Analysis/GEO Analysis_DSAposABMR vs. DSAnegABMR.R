...
# 查看数据集信息
class(gset)  # 查看数据类型
length(gset)  # 查看平台数据集个数
class(gset[[1]])
gset
# assayData:  54613 features, 224 samples
# Annotation: GPL570
...
identical(ids$probe_id, rownames(dat)) # ids与dat全部对齐
...
dat[1:4,1:4]
# dat <- cbind(dat,group_list) # cbind横向追加，即将分组信息追加到最后一列，并除去NO_ABMR
dat1 <- dat %>% t() %>% as.data.frame() %>% cbind(group_list)
dat1 <- dat1[which(group_list != "No ABMRh"),]
table(dat1$group_list)
table(group_list)

dat <- dat1[,-ncol(dat1)] %>% t()
dat[1:4, 1:4]

group_list <- group_list[which(group_list != "No ABMRh")]
table(group_list)


save(dat, group_list, 
     file = 'step1-output_DSApos_vs_DSA_neg.rda')
...
rm(list = ls())
options(stringsAsFactors = F)
load(file = 'step1-output_DSApos_vs_DSA_neg.rda')
table(group_list)
# 每次都要检测数据
dat[1:4,1:4]
## 下面是画PCA的必须操作，需要看说明书。
...
ggsave('all_samples_PCA_grouped_by_DSA.pdf')
dev.off()
...
pheatmap(n, show_colnames =F, show_rownames = F,
         annotation_col=ac,filename = 'heatmap_top200_sd_DSApos_vs_DSAneg.pdf')
dev.off()
...
contrast.matrix <- makeContrasts(paste0(unique(group_list), collapse = "-"), 
                                 levels = design)
contrast.matrix ##这个矩阵声明，我们要把 DSAnegABMRh 跟 DSAposABMRh 进行差异分析比较
...
save(deg, 
     file = 'deg_DSAneg_vs_DSApos.rda')

allDiff <- deg
write.csv(allDiff, file = 'deg_allDiff_DSApos_vs_DSAneg.csv')
diff <- allDiff
#筛选有显著差异的
padj <- 0.05
diffSig <- diff[(diff$P.Value < padj & (diff$logFC>0.5 | diff$logFC<(-0.5))),]

diffUp <- diff[(diff$P.Value < padj & (diff$logFC>0)),]
diffUp <- diffUp[order(diffUp$logFC, decreasing = T),]

diffDown <- diff[(diff$P.Value < padj & (diff$logFC<0)),]
diffDown <- diffDown[order(diffDown$logFC, decreasing = F),]
...
if(T){
  ...
  df$g <- ifelse(df$P.Value>0.01,'stable', # if 判断：如果这一基因的P.Value>0.01，则为stable基因
                 ifelse( df$logFC >0, 'up', #接上句else 否则：接下来开始判断那些P.Value<0.01的基因，再 if 判断：如果logFC >1.5,则为up（上调）基因
                         ifelse( df$logFC < 0, 'down','stable') )#接上句else 否则：接下来开始判断那些logFC <1.5 的基因，再if 判断：如果logFC <1.5，则为down（下调）基因，否则为stable基因
  )
  table(df$g)
  df$name <- rownames(df)
  head(df)
  ...
  ggscatter(df, x = "logFC", y = "v", color = "g",size = 0.5,
            ...
            # label.select = c(head(rownames(diffUp),10), head(rownames(diffDown),10), 'IRF1'), # 挑选一些基因在图中显示出来
            palette = c("#00AFBB", "#E7B800", "#FC4E07") )
  ggsave('volcano_DSAneg_vs_DSApos.pdf')
  dev.off()
  ...
  ggsave('MA_DSAneg_vs_DSApos.pdf')
  dev.off()  
...
if(T){ 
  ...
  pheatmap(n,
           ...
           cluster_cols = F, 
           annotation_col = ac, 
           filename = 'heatmap_top200_DEG_DSAneg_vs_DSApos.pdf') #列名注释信息为ac即分组信息
  
}
dev.off()

write.csv(deg, file = 'deg_DSAneg_vs_DSApos.csv')
...
rm(list = ls())
load(file = 'deg_DSAneg_vs_DSApos.rda')
head(deg)
...
head(DEG)
save(DEG,
     file = 'anno_DEG_DSAneg_vs_DSApos.rda')
...
if(T){
  ...
  kk.up <- enrichKEGG(gene         = gene_up,
                      ...
                      qvalueCutoff =0.9)
  head(kk.up)[,1:6]
  dotplot(kk.up );ggsave('kk.up.dotplot_DSAneg_vs_DSApos.pdf')
  dev.off()
  kk.down <- enrichKEGG(gene         =  gene_down,
                        ...
                        qvalueCutoff =0.9)
  head(kk.down)[,1:6]
  dotplot(kk.down );ggsave('kk.down.dotplot_DSAneg_vs_DSApos.pdf')
  dev.off()
  kk.diff <- enrichKEGG(gene         = gene_diff,
                        organism     = 'hsa',
                        pvalueCutoff = 0.05)
  head(kk.diff)[,1:6]
  dotplot(kk.diff );ggsave('kk.diff.dotplot_DSAneg_vs_DSApos.pdf')
  ...
  ggsave(g_kegg, filename = 'kegg_up_down_DSAneg_vs_DSApos.pdf')
  dev.off()
  ...
  g_kegg <- kegg_plot(up_kegg, down_kegg)
  print(g_kegg)
  ggsave(g_kegg,filename = 'kegg_up_down_gsea_DSAneg_vs_DSApos.pdf')
...
### 做GO数据集超几何分布检验分析，重点在结果的可视化及生物学意义的理解。
{
  ...
  if(F){
    ...
    save(go_enrich_results,file = 'go_enrich_results_DSAneg_vs_DSApos.rda')
    
  }
  
  
  load(file = 'go_enrich_results_DSAneg_vs_DSApos.rda')
  
  n1 <- c('gene_up','gene_down','gene_diff')
  n2 <- c('BP','MF','CC') 
  for (i in 1:3){
    for (j in 1:3){
      fn = paste0('dotplot_',n1[i],'_',n2[j],'_DSAneg_vs_DSApos','.png')
      cat(paste0(fn,'\n'))
...
{
  load(file = 'anno_DEG_DSAneg_vs_DSApos.rda')
  geneList <- DEG$logFC
  ...
  f <- 'gsea_results_DSAneg_vs_DSApos.rda'
  if(!file.exists(f)){
...
## 还有ssGSEA, PGSEA
{
  ...
  save(es_max, es_deg, 
       file='gsva_msigdb_DSAneg_vs_DSApos.rda')
  
  load(file='gsva_msigdb_DSAneg_vs_DSApos.rda')
  ...
  lapply(1:length(es_deg), function(i){
    ...
    if(nrow(df)>5){
      ...
      pheatmap::pheatmap(dat, 
                         ...
                         filename = paste0('gsva_',strsplit(gmts[i],'[.]')[[1]][1],'_DSAneg_vs_DSApos','.pdf'))
  ...
  write.csv(df, file = 'GSVA_DEG_DSAneg_vs_DSApos.csv')
}
